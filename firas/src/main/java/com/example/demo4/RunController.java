...
public class RunController implements Initializable {
    ...
    private void Eleve(ActionEvent ev) {
        try {
            Parent loader = FXMLLoader.load(Objects.requireNonNull(getClass().getResource("Home_front.fxml")));
    ...
    private void Prof(ActionEvent ev) {
        try {
            //navigation
            Parent loader = FXMLLoader.load(Objects.requireNonNull(getClass().getResource("Home_front_Prof_eleve.fxml")));
            idLabel.getScene().setRoot(loader);
        } catch (IOException ex) {
            ex.printStackTrace();
        }
