...
public class questionService implements IquestionService<question> {
    ...
    public void ajouterquestion(question e) throws SQLException {
        ...
        String requete = "INSERT INTO `question` (`name`,`image`,`commentaire`,`type`,`date`) "
                + "VALUES (?,?,?,?,?);";
        try {
    ...
    public Map<String, Integer> countQuestionsByType() throws SQLException {
        ...
        String sql = "SELECT type, COUNT(*) AS count FROM question GROUP BY type";
    ...
    public void modifierquestion(question e) throws SQLException {
        ...
        String req = "UPDATE question SET name = ?,image=?,commentaire = ?,type = ? where id = ?";
    ...
    public  void updateLikes(int questionId) {
        String req = "UPDATE question SET `like` = `like` + 1 WHERE id = ?";
    ...
    public  void updateDislikes(int questionId) {
        String req = "UPDATE question SET `dislike` = `dislike` + 1 WHERE id = ?";
        try (PreparedStatement ps = cnx.prepareStatement(req)) {
            ps.setInt(1, questionId);
            ps.executeUpdate();
    ...
    public int getLikes(int questionId) {
        String req = "SELECT `like` FROM question WHERE id = ?";
        int likes = 0;
        try (PreparedStatement ps = cnx.prepareStatement(req)) {
            ps.setInt(1, questionId);
            ResultSet rs = ps.executeQuery();
    ...
    public int getDislikes(int questionId) {
        String req = "SELECT `dislike` FROM question WHERE id = ?";
        int dislikes = 0;
        try (PreparedStatement ps = cnx.prepareStatement(req)) {
            ps.setInt(1, questionId);
            ResultSet rs = ps.executeQuery();
    ...
    public question FetchOneev(int id) {
        ...
        String requete = "SELECT * FROM `question` where id = " + id;
    ...
    public ObservableList<question> chercherev(String chaine) {
        String sql = "SELECT * FROM question WHERE (name LIKE ? or type_question LIKE ?  ) order by name ";
    ...
    public List<question> trierev() throws SQLException {
        List<question> question = new ArrayList<>();
        String s = "select * from question order by name ";
        Statement st = cnx.createStatement();
        ResultSet rs = st.executeQuery(s);
        while (rs.next()) {
            question e = new question();
